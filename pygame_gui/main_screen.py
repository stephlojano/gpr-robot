...
def draw_first_window(screen):
    ...
    header_text = HEADER_FONT.render(
        "Enter Dimensions of Area to be Scanned (in meters)", 1, PASTEL_BLUE
    )
...
def draw_bottom_banner(screen):
    robot_img = pg.image.load(path.join(".", "assets", "Robot_icon.png"))
...
def manual_move(robot, keys_pressed):
                    ...
                    if ret:
                        try:
                            ...
                            if index == 0:
                                screen.blit(
                                    CHECK_MARK_IMG,
                                    (box.text_pos_x + 300, box.text_pos_y - 10),
                                )
                            else:
                                screen.blit(
                                    CHECK_MARK_IMG,
                                    (box.text_pos_x + 300, box.text_pos_y - 42),
                                )
                        except:
